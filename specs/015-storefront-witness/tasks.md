...
- [ ] T1 — `external_audit` module enablement: per-workspace origin
      assertion (FR-110), server-controlled allowlist, exact-origin
      validation, capability/module reporting.
- [ ] T2 — External target adapter through the public ports: Shopify-shaped
...
- [ ] T3 — Shopify ten-tool contract pack as data; `proceed_to_checkout`
      and `manage_orders` as present-but-never-invoked entries; pack test
      asserting no contract can dispatch them.
- [ ] T4 — Controlled fixture page with a half-broken surface (read path
      works, cart tool lies); e2e audit journey against it.
- [ ] T5 — Merchant-readable report rendering: §5 persona, consequence-first
      summary, engineer-grade evidence behind it.
- [ ] T6 — Guardrail tests: unconfigured module unreachable, non-allowlisted
      origin refused at every layer, no crawling affordance exists.
- [ ] T7 — Product copy: the Shopify context cited as published third-party
