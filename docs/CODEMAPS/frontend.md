...
| `components/GuidanceBanner.tsx` | 192 | "Who acts next", plus the "Go to this step" walk (exports `goToAction`) | The flash class is removed on `animationend`, never a timer; smooth scroll defers to `prefers-reduced-motion`; an `onGo` override lets `App` bring the owning view forward before focus. |
